...
def interleaved_sum(n, odd_func, even_func):
    ...
    #     if x > n:
    #         return 0
    #     return odd_sum(x+2) + odd_func(x)
    
    # def even_sum(x):
    #     if x > n:
    #         return 0
    #     return even_func(x) + even_sum(x+2)

    # return odd_sum(1) + even_sum(1+1)
    def sum_from(k):
        if k > n:
            return 0
        elif k == n:
...
def count_coins(total):
    ...
    def f(total, coin):
        if type(coin) != int:
            ...
            return f(total-coin, coin) + f(total, next_smaller_coin(coin))
...
def make_anonymous_factorial():
    ...
    True
    """
    # return lambda f: lambda n: 1 if n == 1 else n * f(n-1)
