...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    m_left, m_right = left(m), right(m)

    left_torque = length(m_left) * total_mass(end(m_left))
    right_torque = length(m_right) * total_mass(end(m_right))

    if left_torque != right_torque:
        return False
    
    if not is_planet(end(m_left)) and not balanced(end(m_left)):
        return False
    
    if not is_planet(end(m_right)) and not balanced(end(m_right)):
        return False
