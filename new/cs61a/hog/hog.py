...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls > 0:
        ...
        if tmp == 1:
            flag = 1
        sum, num_rolls = sum + tmp, num_rolls - 1
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    opp_tensDigit, play_onesDigit = 0, player_score % 10
    if opponent_score >= 10:
        opp_tensDigit = opponent_score // 10 % 10
    score = abs(opp_tensDigit - play_onesDigit) * 3
    return max(1, score)  
    # END PROBLEM 2
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    tmp = simple_update(num_rolls, player_score, opponent_score, dice)
    return sus_points(tmp)
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score in range(goal+1):
        for opponent_score in range(0, goal+1):
            roll = strategy(score, opponent_score)
            if first_roll == None:
                first_roll = roll
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for i in range(1, 11):
        ...
        # print("DEBUG: %d" % (averaged_dice(i, dice)))
        if avg_score > max_scoring:
            # print("DEBUG: %d" % (averaged_dice(i, dice)))
            min_num_roll = i
            max_scoring = avg_score
        elif avg_score == max_scoring:
            # print("DEBUG: %d" % (averaged_dice(i, dice)))
            min_num_roll = min(i, min_num_roll)
        # print("DEBUG: averaged_dice(%d, dice) = %d \t  max_scoring = %d \t min_num_roll = %d" % (i, averaged_dice(i, dice), max_scoring, min_num_roll))
    return min_num_roll
